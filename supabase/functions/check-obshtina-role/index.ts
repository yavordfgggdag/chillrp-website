// Същата логика като check-site-role / check-police-role – само проверяваме роли Община / Шеф Община
...
const OBSHTINA_MEMBER_ROLE_ID = "1474045899171696906";
const OBSHTINA_CHIEF_ROLE_ID = "1471238721515819104";
...
serve(async (req) => {
  ...
  try {
    ...
    const hasChief = roleIds.includes(normalizeRoleId(OBSHTINA_CHIEF_ROLE_ID));
    const hasMember = roleIds.includes(normalizeRoleId(OBSHTINA_MEMBER_ROLE_ID));
    const role = hasChief ? "chief_obshtina" : hasMember ? "member" : null;
