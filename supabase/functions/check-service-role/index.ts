// Същата логика като check-site-role / check-police-role – само проверяваме роли Механик / Шеф Механик
...
serve(async (req) => {
  ...
  try {
    ...
    const hasChief = roleIds.includes(normalizeRoleId(SERVICE_CHIEF_MECHANIC_ROLE_ID));
    const hasMechanic = roleIds.includes(normalizeRoleId(SERVICE_MECHANIC_ROLE_ID));
