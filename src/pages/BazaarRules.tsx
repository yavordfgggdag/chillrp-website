...
const staticSections: SectionWithId[] = BAZAAR_RULES.map((s, i) => ({ ...s, id: `bazaar-${i}` }));

export default function BazaarRules() {
  const [search, setSearch] = useState("");
  const [sections, setSections] = useState<SectionWithId[]>(staticSections);

  useEffect(() => {
    supabase.from("rule_sections").select("id, page, emoji, title, color, items, note, sort_order").eq("page", "bazaar").eq("is_active", true).order("sort_order", { ascending: true })
      .then(({ data, error }) => {
        if (!error && data && data.length > 0) {
          setSections(data.map((r) => ({ id: r.id, emoji: r.emoji || "", title: r.title, color: r.color || "purple", items: r.items || [], note: r.note ?? null })));
        }
        if (error) console.error("Bazaar rules load error:", error);
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background pt-20">
      <div className="relative py-14 px-4 overflow-hidden">
        <div className="absolute inset-0 bg-neon-yellow/3" />
        <div className="container mx-auto max-w-3xl relative">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-neon-yellow/40 bg-neon-yellow/10 text-neon-yellow text-xs font-heading font-semibold tracking-widest uppercase mb-5">
              <ShoppingBag size={13} /> Базар / Магазин
            </div>
            <h1 className="text-5xl md:text-6xl font-heading font-black tracking-widest uppercase mb-3">
              <span className="text-foreground">Правила</span> <span className="text-neon-yellow">Базар и магазин</span>
            </h1>
          </div>
          <div className="glass border border-neon-yellow/20 rounded-xl p-5 mb-8">
            <div className="text-xs font-heading font-bold tracking-widest uppercase text-neon-yellow mb-3">📌 НА КРАТКО</div>
            <div className="grid sm:grid-cols-2 gap-2 text-sm font-body text-foreground/80">
              <div className="flex items-center gap-2"><span className="text-neon-green">✅</span> Плащания само чрез официалния сайт</div>
              <div className="flex items-center gap-2"><span className="text-neon-green">✅</span> При проблем – тикет в Discord</div>
              <div className="flex items-center gap-2"><span className="text-neon-red">⛔</span> Без измами и споделяне на данни за плащане</div>
            </div>
          </div>
        </div>
      </div>
      <div className="container mx-auto max-w-3xl px-4 pb-20">
        <div className="relative mb-5">
          ...
          <input type="text" placeholder="Търси правило..." value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-11 pr-4 py-3 rounded-xl glass border border-border focus:border-neon-yellow/50 focus:outline-none text-sm font-body text-foreground placeholder:text-muted-foreground bg-transparent" />
        </div>
        <NoCopyWrapper>
        ...
        <div className="mt-8 glass border border-neon-red/30 rounded-xl p-5 text-sm font-body text-foreground/70 leading-relaxed">
          <span className="text-neon-red font-bold">⚠️ СТАФА ЗАПАЗВА ПРАВОТО СИ ДА ПРОМЕНЯ ПРАВИЛАТА</span><br />
          Незнанието не е оправдание. При въпрос за поръчка или плащане отворете тикет в Discord.
        </div>
        </NoCopyWrapper>
      </div>
    </div>
